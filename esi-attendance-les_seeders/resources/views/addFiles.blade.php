...
<div>
    <form action="{{ url('addCSV') }}" method="post" enctype="multipart/form-data" id=fileForm>
        ...
        <br>
        <input multiple type="file" name="csv[]" required>
        <br>
        <br>
        <button type="submit" class="btn btn-outline-secondary" name="sendCSV"><b>Ajouter les fichiers csv</b></button>
    </form>
    <form action="{{ url('addICS') }}" method="post" enctype="multipart/form-data" id=fileForm>
        @csrf
        <h4>Ajout de fichiers ics :</h4>
        <br>
        <input multiple type="file" name="ics[]" required>
        <br>
        <br>
        <button type="submit" class="btn btn-outline-secondary" name="sendICS"><b>Ajouter les fichiers ics</b></button>
    </form>
</div>
@endsection